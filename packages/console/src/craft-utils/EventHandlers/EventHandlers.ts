...
export abstract class EventHandlers<O extends Record<string, any> = any> {
    options: O;
    ...
    addCraftEventListener<K extends keyof HTMLElementEventMap>(
        ...
        el.addEventListener(eventName, bindedListener as any, options);

        return () =>
            el.removeEventListener(eventName, bindedListener as any, options);
    }
        ...
        >(
            (accum, [name, handler]) => ({
                ...
                [name]: (el, required, options) => {
                    ...
                    activeConnectorIds.add(connector?.id as any);
