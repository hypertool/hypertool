...
import { HISTORY_ACTIONS, History } from "./History";
        ...
        ? {
              ...
              history: {
                  ...
                  throttle: (rate?: number) => Delete<
                      ...
                      M extends Options
                          ? M["ignoreHistoryForActions"][number]
                          : never
                  >;
                  merge: () => Delete<
                      {
...
export function useMethods<
    S,
    R extends MethodRecordBase<S>,
    Q extends QueryMethods | null = null,
>(
    methodsOrOptions: MethodsOrOptions<S, R>,
    initialState: any,
    queryMethods?: Q,
    patchListener?: any,
): SubscriberAndCallbacksFor<MethodsOrOptions<S, R>, any> {
    ...
    const [reducer, methodsFactory] = useMemo(() => {
        ...
        return [
            (state: S, action: Action) => {
                const query =
                    queryMethods &&
                    createQuery(queryMethods, () => state, history);

                let finalState;
                // eslint-disable-next-line prefer-const
                let [nextState, patches, inversePatches] = (
                    produceWithPatches as any
                )(state, (draft: S) => {
                    switch (action.type) {
                        ...
                        default:
                            methods(draft, query)[action.type](
                                ...action.payload,
                            );
                    }
                });
                ...
                if (patchListener) {
                    patchListener(
                        ...
                        query,
                        (cb: any) => {
                            const normalizedDraft: any = produceWithPatches(
                                nextState,
                                cb,
                            );
                ...
                throttle: (rate: any) => {
                    return {
                        ...actionTypes
                            .filter(
                                (type) =>
                                    !ignoreHistoryForActions.includes(
                                        type as never,
                                    ),
                            )
                            .reduce((accum, type) => {
