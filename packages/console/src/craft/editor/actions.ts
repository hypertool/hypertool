...
const Methods = (
    ...
    const addNodeTreeToParent = (
        ...
        const iterateChildren = (id: TNodeID, parentId?: TNodeID) => {
    ...
    const getParentAndValidate = (parentId: TNodeID): Node => {
    ...
    return {
         ...
         * @param id
         */
        addLinkedNodeFromTree(tree: INodeTree, parentId: TNodeID, id: string) {
            const parent = getParentAndValidate(parentId);
         ...
         * @param index
         */
        add(nodeToAdd: Node | Node[], parentId: TNodeID, index?: number) {
         ...
         * @param index
         */
        addNodeTree(tree: INodeTree, parentId?: TNodeID, index?: number) {
         ...
         * @param index
         */
        move(selector: NodeSelector, newParentId: TNodeID, index: number) {
            const targets = getNodesFromSelector(state.nodes, selector, {
                existOnly: true,
            });
            ...
            targets.forEach(({ node: targetNode }, i) => {
                ...
                currentParentNodes[currentParentNodes.indexOf(targetId)] =
                    "marked";

                newParent.data.nodes.splice(index + i, 0, targetId);

                state.nodes[targetId].data.parent = newParentId;
                currentParentNodes.splice(
                    currentParentNodes.indexOf("marked"),
                    1,
                );
            });
        },
         ...
         */
        setOptions(cb: (options: Partial<IEditorOptions>) => void) {
            cb(state.options);
        },

        setNodeEvent(
            eventType: TNodeEventType,
            ...
            const nodeIds: Set<TNodeID> = new Set(
         ...
         * @param cb
         */
        setCustom<T extends TNodeID>(
            selector: NodeSelector<NodeSelectorType.Id>,
            cb: (data: IEditorState["nodes"][T]["data"]["custom"]) => void,
        ) {
            const targets = getNodesFromSelector(state.nodes, selector, {
                idOnly: true,
                existOnly: true,
         ...
         * @param dom
         */
        setDOM(id: TNodeID, dom: HTMLElement) {
            if (!state.nodes[id]) {
                return;
            }
         ...
         * @param bool
         */
        setHidden(id: TNodeID, bool: boolean) {
...
export const ActionMethods = (
    ...
    return {
        ...Methods(state, query),
        /*
         * Note: Beaware advanced method! You most likely do not need to use this.
