...
export function NodeHelpers(state: IEditorState, id: TNodeID) {
    invariant(typeof id == "string", ERROR_INVALID_NODE_ID);

    const node = state.nodes[id];

    const nodeHelpers = (id: any) => NodeHelpers(state, id);
        ...
        isDroppable(selector: NodeSelector, onError?: (err: string) => void) {
            ...
            try {
                ...
                targets.forEach(({ node: targetNode, exists }) => {
                    ...
                    if ((currentParentNode as any).id !== newParentNode.id) {
