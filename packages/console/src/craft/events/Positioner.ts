...
export class Positioner {
    ...
    private currentDropTargetId: TNodeID | null;
    // Current closest Canvas Node relative to the currentDropTarget
    private currentDropTargetCanvasAncestorId: TNodeID | null;

    private currentIndicator: Indicator | null = null;

    private currentTargetId: TNodeID | null;
    ...
    constructor(readonly store: TEditorStore, readonly dragTarget: DragTarget) {
    ...
    private getChildDimensions(newParentNode: Node) {
        ...
        return newParentNode.data.nodes.reduce((result, id: TNodeID) => {
            const { dom } = this.store.query.node(id).get();
     ...
     */
    private getCanvasAncestor(dropTargetId: TNodeID) {
        /*
        ...
        const getCanvas = (nodeId: TNodeID): Node | null => {
    ...
    computeIndicator(
        ...
        if (
            ...
            this.isNearBorders(getDOMInfo(newParentNode.dom as any), x, y) &&
