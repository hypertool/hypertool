    ...
    () =>
      <S,>(controller: IHyperController<S>): IHyperContext<S> => ({
        ...
        inflate: (id: string, patches?: Record<string, IPatch>): INode => {
          const fragment = rawRootNode.children.find((node) => node.id === id);
          if (!fragment) {
          ...
          return produce(fragment, (draft) => {
            ...
            const applyPatches = (node: INode) => {
              for (let i = 0; i < node.children.length; i++) {
                ...
                if (child.id in alteredPatches) {
                  const patch = alteredPatches[child.id];
                  ...
                  delete alteredPatches[child.id];
                  applyPatches(child);
                }
              }
            };
